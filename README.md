...
| Cartesia | 已接入 | 支持在线音色列表 | 支持 SSML、速度、音量与 Sonic 情感控制 |
| PlayHT | 已接入 | 支持在线预置音色列表 | 支持多模型、速度与五种音频格式 |

桌面端当前统一注册 **14 家**厂商。厂商能力会同时出现在工作区和本地 REST API 的 `/api/v1/vendors` 响应中。
...
- API 复用桌面端 14 家 Provider、已保存凭证和输出目录，请求体不接受厂商密钥。
...
当前自检覆盖本地 HTTP 契约、鉴权、并发与文件安全、真实 Kestrel 生命周期、烟测脚本、14 家 Provider 请求边界、设置持久化和设置页体验标记。
